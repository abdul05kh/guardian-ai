 ...
 */
export async function scanExternalUrl(url) {
  const prompt = `You are Guardian AI's WebScanner module. Analyze the following URL to see if it hosts copyrighted material that infringes on Guardian AI's protected assets.

URL: ${url}

Given this is an automated scan, provide a JSON response summarizing your findings:
{
 ...
 */
export async function runDeepResearchScan(topic) {
...
Respond ONLY in JSON array of strings: ["Platform A", "Platform B", "Platform C"]`;
  ...
  let step1Result;
  try {
    const rawStep1 = await callGemini(prompt1, { model: 'flash', temperature: 0.2 });
...
Respond ONLY in JSON array of strings: ["Actor1", "Actor2"]`;
  ...
  let step2Result;
  try {
    const rawStep2 = await callGemini(prompt2, { model: 'flash', temperature: 0.3 });
...
Return ONLY valid JSON:
{
  ...
  let step3Result;
  try {
    const rawStep3 = await callGemini(prompt3, { model: 'flash', temperature: 0.1 });
