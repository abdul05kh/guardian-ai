...
export async function callGemini(prompt, options = {}) {
  ...
  try {
    const expectsJson = (prompt && prompt.toLowerCase().includes('json')) ||
      (options.systemInstruction && options.systemInstruction.toLowerCase().includes('json'));
    ...
    if (!response.ok) {
      ...
      return getSimulatedResponse(prompt, options, `API Error: ${response.status} - ${errorMsg}`);
    }
    ...
    if (!data.candidates || data.candidates.length === 0) {
      ...
      return getSimulatedResponse(prompt, options, 'Safety filters blocked the response or candidates array is empty.');
    }

    let textResult = data.candidates[0]?.content?.parts?.[0]?.text || '';
    if (!textResult) return getSimulatedResponse(prompt, options, 'Generated text was empty.');

    // Robust JSON extraction to handle conversational preamble/postamble
    textResult = textResult.replace(/```json/gi, '').replace(/```html/gi, '').replace(/```/g, '').trim();
    ...
    return getSimulatedResponse(prompt, options, `Network or Parsing Error: ${error.message}`);
  }
}
